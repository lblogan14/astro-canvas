...
```jsonc
{
  ...
  "layouts": {
    ...
    "app":       { "sections": [{"title": "Setup", "items": ["promoted:n2.z", "view:v1"]}] },
    "wizard":    { "steps": [{"title": "Load", "items": ["promoted:n1.path"], "nodes": ["n1"]}] },
    "dashboard": { "cols": 12, "row_height": 48,
                   "items": [{"ref": "view:v1", "x": 0, "y": 0, "w": 6, "h": 4}] }
  },
...
`promoted` lifts a node param into the App, Wizard, Dashboard and Batch layouts; `views` pins a
node output. A layout **item** is a ref to one of them — `"promoted:<node>.<param>"` or
`"view:<id>"` (the object forms `{"promoted": …}`, `{"view": …}` and `{"ref": …}` are accepted
too, which is why `layouts.batch.columns` entries parse with the same reader).

| Section | Shape | Notes |
|---|---|---|
| `app` | `{"sections": [{"title", "items", "description?"}]}` | one scrolling form; views in the items land in the right-hand column |
| `wizard` | `{"steps": [{"title", "items", "description?", "nodes?", "optional?"}]}` | `nodes` gates *Next*; it defaults to the nodes the step's items touch |
| `dashboard` | `{"cols": 12, "row_height": 48, "items": [{"ref", "x", "y", "w", "h"}]}` | grid tiles, `cols` 1–48 |
| `batch` | `{"columns", "collect", "max_workers?", "continue_on_error?"}` | see *Batch* below |

`layouts` stays an open mapping: unknown layout names and unknown keys inside a section
round-trip untouched, so adding a field is cheap. `POST`/`PUT /workflows` answer with
**`layout_errors`** — refs that no longer resolve (`bad_layout`, `bad_ref`, `unknown_promoted`,
`unknown_view`, `unknown_node`, `duplicate_view`), each with its `layout`, `ref` and item
`index`. They never block a save: a stale layout is a UI problem, so the document stores fine and
the mode shows the message (`astro_canvas.engine.layouts.layout_issues`). Params and ports are
not re-checked here — the compiler already reports those per node. A layout absent from the
document is generated by the frontend from `promoted` order and `views` (design 8.4).
