...
``rbcodes.IGM.rb_specbin`` and ``rbcodes.utils.compute_SNR_1d`` without their plotting, printing
and file I/O. The nodes prefer the installed ``rbcodes`` when it is importable and use these
kernels otherwise; ``tests/packs/rbcodes/test_kernels_match_rbcodes.py`` asserts both agree.
"""
